...
public class ZaposleniController {
    ...
    private final ZaposleniService servis;

    public ZaposleniController(ZaposleniService servis) {
        this.servis = servis;
    }

    @GetMapping
    public ResponseEntity<ApiResponse<List<ZaposleniDto>>> getAll() {
        return ResponseEntity.ok(ApiResponse.ok(servis.findAll()));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<ZaposleniDto>> getById(
            @PathVariable Integer id) {
        return ResponseEntity.ok(ApiResponse.ok(servis.findById(id)));
    }

    @PostMapping
    public ResponseEntity<ApiResponse<ZaposleniDto>> create(
            @Valid @RequestBody ZaposleniDto dto) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<ZaposleniDto>> update(
            @PathVariable Integer id,
            @Valid @RequestBody ZaposleniDto dto) {
